...
export interface TerrainData {
  ...
  /** 1 where the cell is water (river surface), 0 otherwise. */
  waterMask: Uint8Array;
  river: RiverPath | null;
  minHeight: number;
  maxHeight: number;
  /** Elevation datum considered the river's water surface. */
  seaLevel: number;
}
...
export function generateTerrain(rootPrng: Prng, frame: GeoFrame, config: TerrainConfig): TerrainData {
  const { cols, rows, cellSize, includeRiver } = config;
  ...
  const river = includeRiver ? generateRiver(riverPrng, extent) : null;
